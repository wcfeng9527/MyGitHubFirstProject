...
public class SocketClientRequestThread implements Runnable {
	...
	private static Logger LOGGER = Logger
			.getLogger(SocketClientRequestThread.class);
	...
	public void run() {
		...
		try {
			...
			clientRequest.write(URLEncoder.encode(
					"这是第" + this.clientIndex + " 个客户端的请求11。", "UTF-8")
					.getBytes());
			clientRequest.flush();
			clientRequest.write(URLEncoder.encode(
					"这是第" + this.clientIndex + " 个客户端的请求22。over", "UTF-8")
					.getBytes());
			...
			SocketClientRequestThread.LOGGER.info("接收到来自服务器的信息:" + URLDecoder.decode(message.toString(),"utf-8"));
		} catch (Exception e) {
	...
	public static void main(String[] args) {
		...
		new Thread(new SocketClientRequestThread(countDownLatch,clientIndex)).start();
	}
}
